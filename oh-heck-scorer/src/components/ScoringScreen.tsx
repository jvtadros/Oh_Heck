...
function allTricksRecorded(round: NonNullable<GameRoom['currentRound']>, playerIds: string[]): boolean {
  return playerIds.every((id) => round.tricks[id] !== null && round.tricks[id] !== undefined);
}
...
export function ScoringScreen({ room, playerId }: { room: GameRoom; playerId: string }) {
  ...
  const tricksRecorded = allTricksRecorded(round, playerIds);
  ...
  async function handleSubmitTricks() {
    setBusy(true);
    setError(null);
    try {
      // Save tricks only — stay on scoring so everyone can review round scores
      // and running totals before the host advances.
  ...
  async function handleContinue() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not advance to the next round.');
      setBusy(false);
    }
  }
        ...
        <>
          ...
          {isHost ? (
            <>
              ...
              <PrimaryButton onClick={handleSubmitTricks} disabled={busy || remaining !== 0}>
